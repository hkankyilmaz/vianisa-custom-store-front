...
export const DiaMos = ({url, tags}) => {


  const isMoissanite = tags?.includes('Mos_to_Dia');
  const isLabDiamond = tags?.includes('Dia_to_Mos');
        ...
        <select value={value} onChange={handleChange} className="align-middle leading-[19.5px] w-full h-[41.5px] cursor-pointer bg-transparent px-[15px] py-[10px] focus:border-transparent text-[13px] focus:outline-none border border-[#E5E7EB] z-10">
          {isMoissanite ? (
            <>
              ...
              <option value={`/products/${url.replace("moissanite","diamond")}`} >Diamond</option>
            </>
          ) : isLabDiamond ? (
            <>
              <option value={`/products/${url}`}>Diamond</option>
              <option value={`/products/${url.replace("diamond","moissanite")}`} >Moissanite</option>
            </>
          ) : undefined}
        </select>
